...
class _HomePageState extends State<HomePage> {
          ...
          TextButton(
            onPressed: () async {
              if (editController.text.isNotEmpty) {
                await _firebaseService.updateTripData(docId, {
                  'destination': editController.text,
                });
  ...
  void _deleteTripDialog(BuildContext context, String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'This will permanently clear this profile from the cloud.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  Widget _buildHomeTabView() {
    return const Center(
      child: Text(
        ...
        style: TextStyle(fontSize: 18, color: Color(0xFF2E3D39)),
      ),
    );
  }
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(
                    child: CircularProgressIndicator(color: Color(0xFF2E3D39)),
                  );
                }
                ...
                if (snapshot.hasData) {
                  for (var rawData in snapshot.data!) {
                    ...
                    String endStr =
                        rawData['endDate'] ?? DateTime.now().toIso8601String();

                    DateTime end = DateTime.parse(endStr);
                    bool isHistoryTrip = end.isBefore(DateTime.now());

                    if ((_activeFilter == 'History' && isHistoryTrip) ||
                        (_activeFilter == 'Active' && !isHistoryTrip)) {
                      ...
                      mappedTrips.add(
                        Trip(
                          ...
                          endDate: end,
                          budgetLimit:
                              (rawData['budgetLimit'] as num?)?.toDouble() ??
                              0.0,
                          attractions: List<String>.from(
                            rawData['attractions'] ?? [],
                          ),
                          favoriteLocations: List<String>.from(
                            rawData['favoriteLocations'] ?? [],
                          ),
                          expenses:
                              (rawData['expenses'] as List?)
                                  ?.map(
                                    (e) => ExpenseItem(
                          ...
                          itinerary:
                              (rawData['itinerary'] as List?)
                                  ...
                                  .toList() ??
                              [],
                          imageBytes: rawData['imageBytesString'] != null
                              ? base64Decode(rawData['imageBytesString'])
                              : null,
                        ),
                      );
                    }
                              ...
                              Align(
                                ...
                                child: PopupMenuButton<String>(
                                  ...
                                  onSelected: (value) {
                                    ...
                                    if (value == 'delete')
                                      _deleteTripDialog(context, currentDocId);
                                  },
                                  itemBuilder: (context) => [
                                    const PopupMenuItem(
                                      value: 'edit',
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color: const Color(0xFF2E3D39).withOpacity(0.15),
                              width: 1.5,
                            ),
                          ),
                          child: const Row(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: const QuestBottomNavBar(activePage: 'planner'),
    );
  }
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              ), // ✅ Changed label and icon to match Profile
            ],
          ),
        ),
        Container(
